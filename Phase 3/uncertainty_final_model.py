...
#total loss = loss_mitosis + adaptive_weight * avg_domain_loss
...
#MC Dropout uncertainty hyperparams
#dropout is already in the mitosis head - these control the inference-time behaviour
#MC_NUM_FORWARD_PASSES: how many stochastic samples to draw per batch at test time
#more passes = smoother entropy estimate but slower inference
#UNCERTAINTY_THRESHOLD: predictive entropy cutoff that splits reliable vs ambiguous
#patches above this are flagged for pathologist review
MC_DROPOUT_RATE = 0.3
...
#open the ms coco file after you run 224_patch_around_bbox on train, val, and test sets
#I save them as train_224_patches, val_224_patches, and test_224_patches
...
class MitosisDataset(Dataset):
    def __init__(self, metadata_path, patches_dir, csv_path, split: str = 'train'):
        self.split = split
        ...
        if self.split == 'train':
            ...
            #val/test set gets no augmentation so evaluation is consistent and comparable
        ...
        print(f'[{split}] Loaded {len(self.metadata)} patches ({self.mitotic_count} mitotic, {self.non_mitotic_count} non-mitotic)')
...
#layer4 might encode fine texture which works for 6/7 tumor types but fails for NETs because
...
class DANNModel(nn.Module):
    def __init__(self, num_classes=2, num_domain_classes=None, lambda_val=0.0):
        ...
        #layer2: 56->28, output channels=512
        ...
        self.pool2 = nn.AdaptiveAvgPool2d(1)    #[B,512,1,1]
        self.pool3 = nn.AdaptiveAvgPool2d(1)    #[B,1024,1,1]
        self.pool4 = nn.AdaptiveAvgPool2d(1)    #[B,2048,1,1]
        ...
        #dropout rate is MC_DROPOUT_RATE so the same layer is reused at inference time
        #by keeping dropout active during MC forward passes we get stochastic predictions
        #without adding any extra parameters or changing the training objective
...
#set model to eval (freezes BN, domain heads) but re-enable dropout layers
#so each forward pass through the mitosis head is stochastic
#the domain classifiers are not sampled - we only want uncertainty over mitosis
...
#run n_forward stochastic passes through the mitosis head and aggregate
#returns mean_probs: averaged softmax across all passes [B, 2]
#predictive_entropy: H(E[p]) - total uncertainty per patch [B]
#mutual_info: H(E[p]) - E[H(p)] - model uncertainty (epistemic) per patch [B]
#high predictive_entropy = uncertain overall
#high mutual_info = uncertain because of the model, not just noisy data
def mc_dropout_predict(model, images, n_forward=MC_NUM_FORWARD_PASSES):
    ...
    with torch.no_grad():
        for _ in range(n_forward):
            #only the mitosis head runs here - domain classifiers are skipped
            logits = model.predict_only(images)
            probs = softmax(logits)
            all_probs.append(probs.unsqueeze(0))

    #stack to [n_forward, B, 2] then average over passes
    all_probs = torch.cat(all_probs, dim=0)
    mean_probs = all_probs.mean(dim=0)

    eps = 1e-10
    #predictive entropy: H of the averaged distribution - total uncertainty
    predictive_entropy = -(mean_probs * torch.log(mean_probs + eps)).sum(dim=-1)
    #expected entropy: average H of each individual pass - aleatoric uncertainty
    expected_entropy = -(all_probs * torch.log(all_probs + eps)).sum(dim=-1).mean(dim=0)
    #mutual information: difference = epistemic (model) uncertainty
...
#now also returns all_probs: softmax probability of the mitotic class (index 1) for each patch
#used downstream to compute the ROC curve and AUC
...
#runs MC Dropout inference over the test loader and splits results into
#reliable (entropy < UNCERTAINTY_THRESHOLD) and ambiguous subsets
#ambiguous patches are flagged for pathologist review
#also collects domain labels so we can cross-reference uncertainty by tumor type
def evaluate_with_uncertainty(model, loader, device, n_forward=MC_NUM_FORWARD_PASSES,
    ...
    for images, labels, domain_labels in tqdm(loader, desc='MC Dropout Inference'):
        images = images.to(device)
        ...
        #collect domain labels for downstream cross-referencing
        for attr in domain_attr:
            all_domain_label_list[attr].extend(domain_labels[attr].numpy())
    ...
    #split into reliable and ambiguous based on predictive entropy threshold
    ...
    return all_preds, all_labels, all_entropy, all_mutual_info, all_max_prob, all_domain_label_list


#two-panel plot showing how entropy is distributed across classes and correctness
#left panel: entropy by true label - tells you whether mitotic patches are harder
#right panel: entropy by prediction correctness - confirms that high entropy = wrong prediction
#the dashed threshold line shows the reliable/ambiguous cutoff visually
def plot_uncertainty_distribution(all_entropy, all_labels, all_preds,
    ...
    plt.show()
    print(f"\n Uncertainty distribution plot saved as '{save_path}'")
...
def plot_training_history(history, chance_levels, results_dir='results'):
    ...
    axes[1].plot(epochs, history['test_domain_loss'], 'r-', label='Val Domain Loss', linewidth=2)
...
def main():
    ...
    #val/test sets always get clean transforms so evaluation is consistent across runs
    ...
    #this should boost recall without changing the architecture
    ...
    for epoch in range(num_epochs):
        ...
        history['val_mitosis_loss'] = history.get('val_mitosis_loss', [])
        history['val_mitosis_loss'].append(val_mitosis_loss)
        history['val_domain_loss'] = history.get('val_domain_loss', [])
    ...
    #standard deterministic evaluation first so we have a clean baseline
    ...
    print('\n')
    print('Classification Report (Deterministic)')
    print(classification_report(all_labels, all_preds,
    ...
    plot_uncertainty_distribution(all_entropy, mc_labels, mc_preds,
                                  threshold=UNCERTAINTY_THRESHOLD,
                                  save_path=os.path.join(results_dir, 'uncertainty_distribution.png'))

    #save per-sample results with confidence scores so uncertain patches can be
    #pulled for pathologist review without re-running inference
    ...
    for i in range(len(mc_labels)):
        entry = {
            ...
            'true_label': int(mc_labels[i]),
            'predicted_label': int(mc_preds[i]),
    ...
    out_path = os.path.join(results_dir, 'test_predictions_with_confidence.json')
