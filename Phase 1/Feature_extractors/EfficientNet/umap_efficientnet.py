...
def umap_visualizations():
    # Load features
    with open('./midog_efficientnet_features_patches.pkl', 'rb') as f:
    ...
    for category in categories:
        ...
        plt.title(f'EfficientNet UMAP Projection of Features, Colored by {category}', fontsize=16, pad=20)
        plt.xlabel('UMAP Component 1', fontsize=14)
        plt.ylabel('UMAP Component 2', fontsize=14)
        if category != 'Slide':
            plt.legend(title=category, bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.tight_layout()
        plt.savefig(f'efficientnet_umap_{category.lower()}_patches.png', dpi=300, bbox_inches='tight')
    return final_df

def main():
    #UMAP analysis
    results_df = umap_visualizations()
    results_df.to_csv('umap_results_with_metadata_efficientnet.csv', index=False)
    print("\nUMAP visualization complete!")
    print(f"Results saved to: umap_results_with_metadata_efficientnet.csv")
